...
@section('subtitle', 'Analisis komprehensif, evaluasi akurasi prediksi, dan unduh laporan data retribusi.')
...
<div class="container-fluid p-0 report-container">
    ...
    <x-laporan.filter-toolbar
        ...
        :type="$type ?? 'harian'"
        :readonly="false"
        :pdfRoute="route('operator.laporan.export-pdf', request()->query())"
        :excelRoute="route('operator.laporan.export-excel', request()->query())"
    />
    ...
    <div id="report-content-wrapper">
        ...
        <x-laporan.chart-section
            ...
            :rayonId="$rayonId"
            :type="$type ?? 'harian'"
            :forecastRoute="route('operator.laporan.forecast')"
        />
            ...
            .then(res => {
                if (res.success && res.data) {
                    ...
                    if (canvas) {
                        ...
                        const labels = (data.detail_harian || []).map(day => {
                            if (type === 'harian') {
                                // Shorten e.g. "Senin, 21 Jul 2025" to "Sen, 21 Jul"
                                return day.label.replace(/^([A-Za-z]{3})[a-z]*, (.*) 202\d$/, '$1, $2');
                            }
                            return day.label;
                        });
                            ...
                            options: {
                                ...
                                plugins: {
                                    ...
                                    tooltip: {
                                        ...
                                        callbacks: {
                                            label: function(context) {
                                                return ' Proyeksi: Rp ' + new Intl.NumberFormat('id-ID').format(context.raw);
                                            }
                                        }
                                    }
                                },
