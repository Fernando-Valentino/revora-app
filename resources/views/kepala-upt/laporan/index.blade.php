...
<div class="container-fluid p-0 report-container">
    ...
    <x-laporan.filter-toolbar
        ...
        :type="$type ?? 'harian'"
        :readonly="true"
        :pdfRoute="route('kepala-upt.laporan.export-pdf', request()->query())"
        :excelRoute="null"
    />
    ...
    <div id="report-content-wrapper">
        ...
        <x-laporan.chart-section
            ...
            :rayonId="$rayonId"
            :type="$type ?? 'harian'"
            :forecastRoute="route('kepala-upt.laporan.forecast')"
        />
    ...
    function initLaporanChart() {
        ...
        laporanChartInstance = new Chart(ctx, {
            ...
            data: {
                labels,
                datasets: [
                    { label: 'Realisasi (Aktual)', data: actualValues, borderColor: '#005BAA', borderWidth: 2, backgroundColor: gradientActual, fill: true, tension: 0.3, pointBackgroundColor: '#005BAA', pointBorderColor: '#ffffff', pointBorderWidth: 1, pointRadius: 3, pointHoverRadius: 5 },
                    { label: 'Prediksi', data: predictValues, borderColor: '#F4C542', borderWidth: 2, backgroundColor: gradientPredict, fill: true, tension: 0.3, pointBackgroundColor: '#F4C542', pointBorderColor: '#ffffff', pointBorderWidth: 1, pointRadius: 3, pointHoverRadius: 5, borderDash: [5, 4] }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: {
                    legend: { display: true, position: 'top', align: 'end', labels: { boxWidth: 8, boxHeight: 8, font: { family: 'Inter', size: 11, weight: '500' } } },
                    tooltip: { padding: 10, backgroundColor: 'rgba(15, 23, 42, 0.95)', titleFont: { family: 'Inter', size: 11, weight: 'bold' }, bodyFont: { family: 'Inter', size: 11 }, callbacks: { label: function(c) { return ' ' + c.dataset.label + ': Rp ' + new Intl.NumberFormat('id-ID').format(c.raw); } } }
                },
                scales: {
                    y: { grid: { borderDash: [6, 6], color: '#e2e8f0', drawBorder: false }, ticks: { callback: function(v) { return 'Rp ' + new Intl.NumberFormat('id-ID', { notation: 'compact' }).format(v); }, font: { family: 'Inter', size: 10 } } },
        ...
        fetch(`${forecastUrl}?rayon_id=${rayonId}&type=${type}&model_type=${modelType}`)
            ...
            .then(res => {
                if (res.success && res.data) {
                    ...
                    recsList.innerHTML = '';
                    (data.recommendations && data.recommendations.length > 0 ? data.recommendations : ['Tidak ada rekomendasi spesifik.']).forEach(rec => {
